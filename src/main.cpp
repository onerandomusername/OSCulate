...
void
sendRemoteCommand (const char key[], bool isDown)
{
...
void
OnHIDExtrasPress (uint32_t top, uint16_t key)
{
#ifdef KEYBOARD_INTERFACE
  if (top == 0xc0000)
    {
...
void
OnHIDExtrasRelease (uint32_t top, uint16_t key)
{
#ifdef KEYBOARD_INTERFACE
  if (top == 0xc0000)
    {
...
string
rawKeyToString (uint8_t keycode)
{
...
void
ShowHIDExtrasPress (uint32_t top, uint16_t key)
{
#ifdef SHOW_KEYBOARD_DATA
  Serial.print ("HID (");
  Serial.print (top, HEX);
